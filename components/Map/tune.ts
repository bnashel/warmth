...
export const PERF = {
  ...
  restFrameMs: 33,
  /** LOW-TIER FIELD SCALE (perf pass, 2026-07-14): on genuinely weak
   *  phones the field's offscreen buffers render at resolutionScale ×
   *  this. The accumulation targets hold soft coreless bells that the
   *  resolve upsamples bilinearly, so the cut is invisible at phone
   *  viewing distance while the splat fill — the #1 low-end cost —
   *  drops ~44%. Buffer resolution ONLY; visible character dials must
   *  never key off the device tier. */
...
export function isLowTierDevice(gl?: WebGL2RenderingContext): boolean {
  ...
  try {
    ...
    // Mali-4xx/T-series, entry Mali-G5x, budget PowerVR GE8xxx).
    const dbg = gl.getExtension("WEBGL_debug_renderer_info");
    const renderer = String(
      dbg ? gl.getParameter(dbg.UNMASKED_RENDERER_WEBGL) : gl.getParameter(gl.RENDERER),
    );
    lowTierCache =
      /Adreno[^\d]*[1-5]\d\d\b|Mali-4\d\d|Mali-T\d|Mali-G5[127]\b|PowerVR Rogue GE8/i.test(
        renderer,
      );
  } catch {
    lowTierCache = false; // the sniff is best-effort — never a blocker
  }
  return lowTierCache;
}
